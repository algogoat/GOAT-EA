...
| DIST-001 | One-download EA, controller, builder, optimization suites and strategy/asset matrix | [Delivery specification](docs/operations/GOAT-SETUP-BUNDLE.md); implementation pending | Signed installer; selected-terminal discovery/install/repair; bundled runtimes and portable agent guide; exact suite/matrix links and compatibility; clean Windows first-use optimization-to-portfolio workflow; preserve user files and run state |
| MATRIX-001 | Living strategy/asset matrix and independent GOAT strategy-library updates | [Data and update contract](docs/operations/GOAT-SETUP-BUNDLE.md#living-matrix-and-ongoing-releases); implementation pending | Append verified local outcomes including failures; exact settings/run/evidence identities; separate publisher and user provenance; immutable template revisions, compatible atomic catalog updates, preserved user forks and frozen jobs, rollback; no implicit result upload or new optimization |

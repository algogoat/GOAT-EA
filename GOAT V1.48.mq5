...
bool  g_PerformanceProfileTester=false;
ulong g_PerfLotSolverUS=0,g_PerfLotSolverCalls=0;
ulong g_PerfMLPSUS=0,g_PerfMLPSCalls=0;
ulong g_PerfTrailingUS=0,g_PerfTrailingCalls=0;
ulong g_PerfSignalUS=0,g_PerfSignalCalls=0;
ulong g_TSLBuySkipped=0,g_TSLSellSkipped=0;

void GoatPerformanceReset()
  {
   g_PerfLotSolverUS=g_PerfLotSolverCalls=0;
   g_PerfMLPSUS=g_PerfMLPSCalls=0;
   g_PerfTrailingUS=g_PerfTrailingCalls=0;
   g_PerfSignalUS=g_PerfSignalCalls=0;
   g_TSLBuySkipped=g_TSLSellSkipped=0;
  }

void GoatPerformanceRecord(ulong &total_us,ulong &calls,const ulong started_us)
  {
   if(!g_PerformanceProfileTester) return;
   total_us+=GetMicrosecondCount()-started_us;
   calls++;
  }

void GoatPerformancePrintSummary()
  {
   if(!g_PerformanceProfileTester) return;

   double lot_avg=(g_PerfLotSolverCalls>0) ? (double)g_PerfLotSolverUS/(double)g_PerfLotSolverCalls : 0.0;
   double mlps_avg=(g_PerfMLPSCalls>0) ? (double)g_PerfMLPSUS/(double)g_PerfMLPSCalls : 0.0;
   double trailing_avg=(g_PerfTrailingCalls>0) ? (double)g_PerfTrailingUS/(double)g_PerfTrailingCalls : 0.0;
   double signal_avg=(g_PerfSignalCalls>0) ? (double)g_PerfSignalUS/(double)g_PerfSignalCalls : 0.0;

   PrintFormat("GOAT PERF SUMMARY: lot_solver_us=%I64u calls=%I64u avg_us=%.2f; mlps_us=%I64u calls=%I64u avg_us=%.2f; trailing_us=%I64u calls=%I64u avg_us=%.2f; signal_us=%I64u calls=%I64u avg_us=%.2f; tsl_skipped_buy=%I64u tsl_skipped_sell=%I64u",
               g_PerfLotSolverUS,g_PerfLotSolverCalls,lot_avg,
               g_PerfMLPSUS,g_PerfMLPSCalls,mlps_avg,
               g_PerfTrailingUS,g_PerfTrailingCalls,trailing_avg,
               g_PerfSignalUS,g_PerfSignalCalls,signal_avg,
               g_TSLBuySkipped,g_TSLSellSkipped);
  }
...
//---------------------- solver for StartLots by target sequence loss (MLPS)
   ...
   {
    ulong profile_started=g_PerformanceProfileTester ? GetMicrosecondCount() : 0;
    double vmin  = SymbolInfoDouble(_Symbol,SYMBOL_VOLUME_MIN);
    double vmax  = SymbolInfoDouble(_Symbol,SYMBOL_VOLUME_MAX);
    double vstep = SymbolInfoDouble(_Symbol,SYMBOL_VOLUME_STEP);

    if(vstep <= 0.0 || vmax <= 0.0) {GoatPerformanceRecord(g_PerfLotSolverUS,g_PerfLotSolverCalls,profile_started); return vmin;}
    if(targetLoss<=0.0) {GoatPerformanceRecord(g_PerfLotSolverUS,g_PerfLotSolverCalls,profile_started); return vmin;}
    ...
    double result=NormalizedLots(best);
    GoatPerformanceRecord(g_PerfLotSolverUS,g_PerfLotSolverCalls,profile_started);
    return result;
   }
...
//---------------------- optional live MLPS breach stop for any real sequence
   ...
   {
    ulong profile_started=g_PerformanceProfileTester ? GetMicrosecondCount() : 0;
    if(!Sequence_MLPS_Hard_Close) {GoatPerformanceRecord(g_PerfMLPSUS,g_PerfMLPSCalls,profile_started); return false;}
    if(Virtual || !Active || !Traded) {GoatPerformanceRecord(g_PerfMLPSUS,g_PerfMLPSCalls,profile_started); return false;}
    if(Risk<=0.0) {GoatPerformanceRecord(g_PerfMLPSUS,g_PerfMLPSCalls,profile_started); return false;}
    ...
    if(MQLInfoInteger(MQL_OPTIMIZATION))
    {
     ...
     if(EstimateSequencePLFromCache(estimatedPL) && estimatedPL>(-Risk+exactBand))
       {GoatPerformanceRecord(g_PerfMLPSUS,g_PerfMLPSCalls,profile_started); return false;}
    }

    double pl=CurrentSequencePL();
    if(pl>-Risk) {GoatPerformanceRecord(g_PerfMLPSUS,g_PerfMLPSCalls,profile_started); return false;}
    ...
    GoatPerformanceRecord(g_PerfMLPSUS,g_PerfMLPSCalls,profile_started);
    return true;
   }
...
//----------------------
   void TrailingStoploss()
   {
    ...
    GoatPerformanceRecord(g_PerfTrailingUS,g_PerfTrailingCalls,profile_started);
   }
//------------------------------------------------------------------
...
void OnDeinit(const int reason)
 ...
 //temp="--------------------------------------------------------------------------------------------------------"; Print(temp); desc+="; "+temp+"\n";
   ...
   temp= "Orders="        +(string)(orders-orderErrors)          +"/"+(string)orders
        ...
        +" TSL_Skipped_Buy="+(string)g_TSLBuySkipped+" TSL_Skipped_Sell="+(string)g_TSLSellSkipped
...
//-------
    ...
    bool control_tower_v2=GOATUsingControlTowerBias();
...
//-------------------------------------------------------------------------
   if( CheckSignalNow() )//|| Check_Forced)
   {
    ...
    GoatPerformanceRecord(g_PerfSignalUS,g_PerfSignalCalls,profile_started);
   }
...
bool CPanelDialog::Create(const long chart,const string name,const int subwin,const int x1,const int y1,const int x2,const int y2)
 ...
 //--- create dependent controls
   ...
   if(!CreateButton(m_button_DB,"Dashboard",22,BUTTON_WIDTH*4,24,true,GOAT_UI_BUTTON_BACK,GOAT_UI_SECTION_BORDER,GOAT_UI_SECTION_TEXT,Font_Size,Font_Text)) return false;
   Vertical_Pointer+=GAP_Y;
...
void CPanelDialog::OnClickBackToDB(void)
  {
   m_button_DB.Pressed(false);
   // A chart ID is a hint, not proof. Inspect the actual dashboard caption;
   // never fall back to the first strategy chart or create a second dashboard.
   long dashboard_cid=GoatFindDashboardChart(Key);
   if(dashboard_cid<0)
     {
      MessageBox("No portfolio dashboard is open in this terminal. Open or resume the saved dashboard first.","Dashboard",MB_OK|MB_ICONINFORMATION);
      return;
     }
   if(!ChartSetInteger(dashboard_cid,CHART_BRING_TO_TOP,0,true))
      PrintFormat("Dashboard navigation failed: chart=%I64d error=%d",dashboard_cid,GetLastError());
   else ChartRedraw(dashboard_cid);
  }
//+------------------------------------------------------------------+

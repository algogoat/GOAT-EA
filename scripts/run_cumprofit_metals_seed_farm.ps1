[CmdletBinding()]
param(
    [string]$SuiteDir = "C:\Users\web\AppData\Roaming\MetaQuotes\Terminal\Common\Files\GOAT OPT Files\V1.41 CUMProfit",
    ...
    [string]$VersionLabel = "GOAT V1.41",
    [string[]]$Symbols = @("XAUUSD", "XAGUSD"),
    [int]$FrameTarget = 256,
    [string]$FromDate = "2025.06.02",
    [string]$ToDate = "2026.05.25",
    [string]$Period = "M1",
    [string]$ReportSubdir = "CumProfitMetals",
    [string]$ProgressCsvName = "cumprofit_metals_progress.csv",
...
function Find-SeedXml {
    ...
    $pattern = "$VersionLabel $Symbol,$Period $FromDate-$ToDate $StrategySafe" + "_N$FrameTarget" + "_*.xml"
...
if (-not (Test-Path -LiteralPath $makeConfig -PathType Leaf)) {
    throw "Missing config builder: $makeConfig"
}
if (-not (Test-Path -LiteralPath $TerminalPath -PathType Leaf)) {
    throw "Missing terminal: $TerminalPath"
}
...
foreach ($set in $sets) {
    ...
    foreach ($symbol in $Symbols) {
        $index++
        $strategy = "${baseDesc}_${symbol}_${RunLabel}"
        ...
        Write-Host "[$index/$total] START $($set.Name) $symbol,$Period Risk=$risk Run=$RunLabel"
        python $makeConfig `
...
Write-Host "CumProfit metals seed farming complete. Progress CSV: $progressCsv"

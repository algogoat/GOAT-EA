[CmdletBinding()]
param(
    [string]$SuiteDir = "C:\Users\web\AppData\Roaming\MetaQuotes\Terminal\Common\Files\GOAT OPT Files\V1.39 Smart Peak News MLPS 400 Retune Candidates",
    ...
    [string]$Period = "M1",
    [string]$ReportSubdir = "SmartPeakRetuneEURUSD",
    [string]$ProgressCsvName = "smart_peak_retune_eurusd_progress.csv",
...
foreach ($set in $sets) {
    ...
    $existing = Find-SeedXml -StrategySafe $strategySafe
    if ($Resume -and $existing) {
        Write-Host "[$index/$total] SKIP existing $($set.Name) $Symbol -> $($existing.Name)"
        Add-ProgressRow -SetFile $set.Name -Strategy $baseDesc -Risk $risk -Status "SkippedExisting" -DurationMinutes 0 -XmlPath $existing.FullName -Message "Resume found existing XML"
        continue
    }
    ...
    Write-Host "[$index/$total] START $($set.Name) $Symbol,$Period Risk=$risk"
    python $makeConfig `
    ...
    while ((Get-Date) -lt $deadline) {
        ...
        $xml = Find-SeedXml -StrategySafe $strategySafe
        if ($xml) {
            $status = "Completed"
    ...
    Add-ProgressRow -SetFile $set.Name -Strategy $baseDesc -Risk $risk -Status $status -DurationMinutes $duration -XmlPath $xmlPath -Message $message
    Write-Host "[$index/$total] $status $($set.Name) $Symbol,$Period $([math]::Round($duration,2))m"
}

Clear-LocalGoatSeedFarmingLeftovers
Write-Host "EURUSD retune seed farming complete. Progress CSV: $progressCsv"
